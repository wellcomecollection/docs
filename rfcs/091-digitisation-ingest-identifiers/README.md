...
- [RFC 088: Migrating identity, requesting and items APIs from Sierra to FOLIO](../088-folio-identity-requesting-migration/README.md): the wider identity and requesting half of the same Sierra→Folio migration.
- [RFC 090: CMS to LMS Sync](../090-axiell-folio-sync/README.md): the adjacent CMS↔LMS synchronisation work (open in [PR #157](https://github.com/wellcomecollection/docs/pull/157)).
...
| **IIIF / iiif-builder** | The manifest URI is keyed on the b-number and is self-identifying (e.g. `https://iiif.wellcomecollection.org/presentation/b28047345`, live, HTTP 200, not redirected to a Work id). The DLCS asset id is `{b-number}_{seq}` (e.g. `b28047345_0032`). iiif-builder loads the package by b-number. Legacy `wellcomelibrary.org/iiif/b…/manifest` URLs 301-redirect to the b-number URLs, so they are treated as permanent and must keep resolving. | — |
...
**Collapse the METS handle onto the public Work id.** We could make the METS work's canonical id the same as the public Work id. Rejected: it departs from how METS works behave today (`mets/<b-number>` already mints a canonical id distinct from the Sierra work's), and the merge mechanism does not require it — the merge runs through the merge candidate, not through a shared canonical id. See [METS works keep their own id](#mets-works-keep-their-own-id).

**Rely on plain resolve-or-mint with no verification.** The pre-minting (the app mints `mets/<folio-id>` → C before ingest) could be left to stand on its own, trusting the pipeline id_minter to look the source id up and find the same C. Rejected: a resolve-or-mint that misses — a METS work whose source identifier was never pre-minted, or was minted to a different id — would mint a fresh canonical id, silently diverging the work's id from the storage handle C it was ingested under, which is the silent failure mode this design is trying to avoid. The proposal instead has the transformer supply C (read from the storage `externalIdentifier`, not embedded in the METS XML) and the id_minter verify the `mets/<folio-id>` ↔ C binding, raising on a mismatch. See [METS works keep their own id](#mets-works-keep-their-own-id).
...
3. the id_minter verifying the `mets/<folio-id>` ↔ C binding and failing loudly rather than minting ([METS works keep their own id](#mets-works-keep-their-own-id)), and
...
1. **Storage identifier and record identifier diverge.** Post-migration, the storage `externalIdentifier` (C) is no longer the same string as the METS record identifier (`folio-id`). Today they are both the b-number, and parts of the estate may assume so. The split is mechanically sound — the METS adapter keys on the `externalIdentifier`, while the transformer reads the record identifier from inside the METS XML — but we have not confirmed that iiif-builder and DLCS make no assumption that the two are equal, given DLCS derives its asset ids from the package id and the transformer derives the METS own-identity from the record identifier. This should be checked before the post-migration path ships.
2. **The id_minter can verify a supplied canonical id.** The fail-loud guard in [METS works keep their own id](#mets-works-keep-their-own-id) needs the pipeline id_minter to accept a pre-assigned canonical id for a source identifier, verify the `mets/<folio-id>` ↔ C binding, and raise on a mismatch or absence rather than assigning a new one. Confirm the id_minter can do this, or scope the change needed, before the guard is implemented.
...
4. **Add the post-migration transformer work.** Add the `folio-instance` branch to the METS transformer (`MetsMergeCandidate`, `MetsData`), have the transformer carry the pre-minted canonical id C from the storage `externalIdentifier` into the work's identifier block, and add the fail-loud guard so the id_minter verifies the `mets/<folio-id>` ↔ C binding rather than minting a divergent id.
...
7. **Resolve the open questions.** Confirm iiif-builder and DLCS tolerate the storage `externalIdentifier` differing from the METS record identifier, and confirm the id_minter can verify a supplied source-id ↔ canonical-id binding (raising rather than minting), before the post-migration path ships.
