...
The contract lives with the code it describes, in
[`catalogue-api/identifiers/spec`](https://github.com/wellcomecollection/catalogue-api/tree/main/identifiers/spec):

- **[`openapi.yaml`](https://github.com/wellcomecollection/catalogue-api/blob/main/identifiers/spec/openapi.yaml):**
  the OpenAPI 3.0 specification (the source of truth).
- **[`openapi.md`](https://github.com/wellcomecollection/catalogue-api/blob/main/identifiers/spec/openapi.md):**
  a human-readable rendering of the same spec, generated from it.

It moved there with the implementation, so that the API Gateway body can template it from the same
repository and there is no copy to keep in step. This RFC previously carried its own copy.
...
`openapi.md` is generated by `render_docs.py` alongside the spec, which also validates it. That runs
on every pull request in catalogue-api.
