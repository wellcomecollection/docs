...
In the first iteration of the graph pipeline, label-derived nodes with the type `Concept` will be matched to `SourceConcept` nodes via their label and added as `HAS_SOURCE_CONCET` edges to these as part of the first iteration of the graph pipeline. Further analysis will be made on how to match other label-derived concept types (e.g. `Person`), which may be more ambiguous. In addition, there are label-derived concepts which are similar, but not exactly the same as another concept from an external ontology. A machine learning algorithm can be used to perform record linkage (entity resolution) on these label-derived concepts to match them to external sources in the future. The proposed graph model (see [RFC #64](../064-graph-data-model/README.md)) and pipeline design will enable us to apply different matching algorithms and/or rules to subsets of label-derived concepts and store the results in the graph together with the source of the match (e.g. `label`, `ml-derived` etc.). This will make it straightforward to filter and selectively enable/disable merging of concepts.
