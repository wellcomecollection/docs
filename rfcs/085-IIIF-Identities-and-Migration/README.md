...
- `GET /identifiers/v1/{canonicalId}` returns the full set of source identifiers for a canonical id;
- `GET /identifiers/v1/by-source/{sourceSystem}/{value}?include=siblings` resolves a source identifier to its canonical id and labelled siblings (`sierra-system-number`, `calm-ref-no`, `folio-instance`, ...).
